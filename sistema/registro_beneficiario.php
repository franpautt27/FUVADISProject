...
include "../conexion.php";
    if(!empty($_POST)){
        $alert='';
        if(empty($_POST['primer_nombre']) || empty($_POST['segundo_nombre']) || empty($_POST['primer_apellido']) || empty($_POST['segundo_apellido']) || empty($_POST['nacionalidad']) || empty($_POST['motivo1']) ||  empty($_POST['estatus_migratorio']) || empty($_POST['tipo_documento']) || empty($_POST['genero']) || empty($_POST['email']) || empty($_POST['forma_empleo']) || empty($_POST['fecha_nacimiento']) || empty($_POST['rango_ingresos']) || empty($_POST['num_comidas_dias']) || empty($_POST['lgbti']) || empty($_POST['trabajo_sexual']) || empty($_POST['condicion_salud']) || empty($_POST['departamento']) || empty($_POST['municipio']) || empty($_POST['direccion']) || empty($_POST['barrio']) || empty($_POST['situacion_vivienda']) || empty($_POST['telefono']) || empty($_POST['n_family'])    || empty($_POST['descripcion'])  || empty($_POST['necesidad1']) ){
            ...
            $result=0;    
            if(is_numeric($num_documento)){
                $query = mysqli_query($conection, "SELECT * FROM registro_beneficiarios WHERE num_documento = '$num_documento' AND tipo_documento = '$tipo_documento'");
                $result = mysqli_fetch_array($query);
            }
            if($result>0){
                $alert = '<p class="msg_error">El numero de documento ya existe<p/>';
            }else{
                $query_insert = mysqli_query($conection, "INSERT INTO registro_beneficiarios(primer_nombre,
                segundo_nombre,
               primer_apellido,
               segundo_apellido,
               motivo1,
               ...
               '$descripcion',
                ...
                if($query_insert){
                    $alert = '<p class="msg_save">Potencial beneficiario guardado correctamente<p/>';
                }else{
                    $alert = '<p class="msg_error">Error al guardar al potencial beneficiario<p/>';
                }
...
<?php include "includes/header.php"; ?>
	<?php include "../formulario/preguntas.php"; ?>
